...
cols_ = c(id_cols, target_, cols_features)
task_ret_q <- as_task_regr(DT[, ..cols_],
                           id = "task_ret_q",
                           target = target_)
...
task_ret_month2$col_roles$feature = setdiff(task_ret_month2$col_roles$feature,
                                            id_cols)
task_ret_q$col_roles$feature = setdiff(task_ret_q$col_roles$feature,
                                       id_cols)
...
# generate cv's
train_sets = seq(10, 50, 10)
validation_sets = train_sets / 10
custom_cvs = list()
...
lapply(custom_cvs[5:length(custom_cvs)], function(cv_) {
# for (cv_ in custom_cvs) {
  ...
  for (i in 26:custom_inner$iters) {
    # debug
    # i = 1
    print(i)
    ...
    design = benchmark_grid(
      tasks = list(task_ret_week, task_ret_month, task_ret_q), # task_ret_month2, , task_ret_month
