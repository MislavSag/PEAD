...
mlr3_save_path = "D:/mlfin/cvresults-pead-v1"
...
learners_l = list(
  ...
  # nnet = lrn("regr.nnet", id = "nnet") # seems too slow
)
...
graph =
  ...
  # filters
  po("branch", options = c("jmi", "gausscov"), id = "prep_branch") %>>%
  gunion(list(po("filter", filter = flt("jmi"), filter.frac = 0.05), po("filter", filter = flt("gausscov_f1st"), filter.cutoff = 0))) %>>%
...
# define search space
as.data.table(graph_learner$param_set)[1:100, .(id, class, lower, upper, levels)]
...
search_space = ps(
  ...
  # interaction terms
  prep_branch.selection = p_fct(levels = c("jmi", "gausscov")),
  # learners
  ranger.ranger.replace = p_lgl(),
  ranger.ranger.splitrule = p_fct(levels = c("variance", "extratrees")),
  ranger.ranger.mtry.ratio = p_dbl(0.7, 1)
  # ranger.ranger.num.trees
)
