...
factors = Factors$new()
factors_l = factors$get_factors(first_date = as.Date("2018-01-01"))
...
# remove price object to save memory
rm(prices_dt)
gc()
...
# get at_ for Ohlcv featuers
prices_events <- merge(OhlcvInstance$X,
                       dataset[, .(symbol, date, eps)],
                       by = c("symbol", "date"), all.x = TRUE, all.y = FALSE)
...
OhlcvFeaturesSetSample_new <- OhlcvFeaturesSet[at_ - lag_]
setorderv(OhlcvFeaturesSetSample_new, c("symbol", "date"))

# get only new data and merge
OhlcvFeaturesSetSample_new[, date := as.IDate(date)]
OhlcvFeaturesSetSampleNew = fsetdiff(OhlcvFeaturesSetSample_new[, .(symbol, date)],
                                     OhlcvFeaturesSetSample[, .(symbol, date)])
OhlcvFeaturesSetSampleNew[, index := 1]
OhlcvFeaturesSetSampleNew = OhlcvFeaturesSetSampleNew[OhlcvFeaturesSetSample_new, on = c("symbol", "date")]
OhlcvFeaturesSetSampleNew = OhlcvFeaturesSetSampleNew[index == 1]
dim(OhlcvFeaturesSetSample)
OhlcvFeaturesSetSampleNew = OhlcvFeaturesSetSampleNew[, .SD, .SDcols = colnames(OhlcvFeaturesSetSample)]
OhlcvFeaturesSetSample = rbind(OhlcvFeaturesSetSample, OhlcvFeaturesSetSampleNew)
dim(OhlcvFeaturesSetSample)
...
RollingTheftTsfelFeaturesNewMerged = rbind(RollingTheftTsfelFeatures, RollingTheftTsfelFeaturesNew)
time_ <- format.POSIXct(Sys.time(), format = "%Y%m%d%H%M%S")
...
features[, .(symbol, date_rolling, date_ohlcv)]
